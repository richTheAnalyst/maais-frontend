...
import {
  Settings2, BookOpen, Clock, ShieldCheck, AlertTriangle,
  ChevronRight, Gauge, Palette, History, Info, ShieldAlert,
  Sparkles, Lock, Unlock, Loader2, RefreshCw, CheckCircle2,
  Users, FileCheck
} from 'lucide-react';
...
const toggleStudentLock = async (student: ClassSummaryItem) => {
  ...
  try {
    if (student.hasAnyLocked) {
      await api.post('/grading/entries/bulk-unlock', { ids: student.gradeEntryIds });
      showToast(`Unlocked grades for ${student.name}`);
    } else {
      await Promise.all(
        student.gradeEntryIds.map(id => api.patch(`/grading/entries/${id}/lock`))
      );
...
const handleBulkLockClass = async () => {
  if (!window.confirm('Lock ALL grade entries for this class? Teachers will be unable to edit any of them.')) return;
  ...
  try {
    ...
    await Promise.all(allIds.map(id => api.patch(`/grading/entries/${id}/lock`)));
...
const totalLocked = classSummary.filter(s => s.hasAnyLocked).length;
  ...
  const handleBulkApproveClass = async () => {
    ...
    try {
      ...
      for (const student of studentsInClass) {
        try {
          const res = await api.get(`/grading/students/${student.id}/terms/${activeTerm.id}`);
          const unapproved = res.data.filter((g: any) => !g.isApproved).map((g: any) => g.id);
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <p className="text-sm font-bold text-slate-400">Loading grading protocol...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex-1 p-8 bg-slate-50 overflow-y-auto relative">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-gray-500 text-center text-sm font-medium leading-relaxed mb-8">
                This will <span className="font-black text-rose-600">permanently freeze</span> all marks and assessments for this term. Teachers will no longer be able to edit grades.
              </p>
              ...
              <div className="space-y-4 mb-10">
                ...
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <p className="text-sm font-black text-slate-900">
                    {fullyApproved} of {totalStudents} students fully approved ({selectedClassId ? classes.find(c => c.id === selectedClassId)?.name : ''})
                  </p>
                </div>
              </div>
              ...
              {fullyApproved < totalStudents && totalStudents > 0 && (
                <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 mb-6 flex items-start gap-3">
                  ...
                  <p className="text-xs font-bold text-amber-700">
                    Some students in the selected class still have unapproved grades. Locking the term anyway will freeze them as-is.
                  </p>
                </div>
              )}
      ...
      <div className="max-w-7xl mx-auto space-y-10 pb-20">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <button onClick={fetchData} className="flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-600 hover:bg-slate-50 transition-all">
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-100">
                      ...
                      <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Remark Category</th>
                      <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Smart Remarks Pool</th>
                    </tr>
                  </thead>
                    ...
                    {boundaries.map(b => (
                      <tr key={b.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-10 py-5">
                          <span className="text-[13px] font-black font-mono text-slate-700">{b.min}–{b.max}</span>
                        </td>
                        ...
                        <td className="px-10 py-5">
                          <div className="flex flex-wrap gap-1.5 max-w-md">
                            {b.smartRemarks.slice(0, 2).map((r, i) => (
                              <span key={i} className="px-2 py-1 bg-slate-50 border border-slate-100 rounded-lg text-[10px] font-medium text-slate-500 italic">
                                {r}
                              </span>
                            ))}
  ...
  <div className="bg-white p-4 rounded-2xl border border-slate-100 text-center">
    ...
    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Students</p>
  </div>
  <div className="bg-white p-4 rounded-2xl border border-slate-100 text-center">
    <p className="text-2xl font-black text-emerald-600">{fullyApproved}</p>
    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Approved</p>
  </div>
  <div className="bg-white p-4 rounded-2xl border border-slate-100 text-center">
    <p className="text-2xl font-black text-rose-600">{totalLocked}</p>
    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Locked</p>
  </div>
  <div className="bg-white p-4 rounded-2xl border border-slate-100 text-center">
    <p className="text-2xl font-black text-slate-900">{avgProgress.toFixed(0)}%</p>
    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Progress</p>
  </div>
</div>
              ...
              <div className="max-h-96 overflow-y-auto">
                {isLoadingSummary ? (
                  ...
                  <div className="py-16 text-center">
                    ...
                    <p className="text-sm font-bold text-slate-400">No students or grades found for this class</p>
                  </div>
                ) : (
  ...
  <div key={s.id} className="flex items-center justify-between p-5 hover:bg-slate-50 transition-colors">
    <div className="flex items-center gap-4 flex-1">
      <div className={cn(
        ...
        s.isFullyApproved ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
      )}>
        {s.isFullyApproved ? <ShieldCheck size={16} /> : <Clock size={16} />}
      </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {s.hasAnyLocked && (
            <span className="flex items-center gap-1 px-1.5 py-0.5 bg-rose-100 text-rose-600 rounded-md text-[8px] font-black uppercase tracking-widest shrink-0">
              <Lock size={8} /> {s.isFullyLocked ? 'Locked' : 'Partial'}
            </span>
          )}
        </div>
        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{s.indexNumber}</p>
      </div>
      <div className="w-32 shrink-0">
        <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
          <div
            className={cn('h-full rounded-full', s.isFullyApproved ? 'bg-emerald-500' : 'bg-amber-400')}
    ...
    <div className="flex items-center gap-3 ml-4">
      ...
      <button
        ...
        disabled={lockingStudentId === s.id || s.gradeEntryIds.length === 0}
        className={cn(
        ...
        title={s.hasAnyLocked ? 'Unlock grades' : 'Lock grades'}
      >
        {lockingStudentId === s.id
          ? <Loader2 size={14} className="animate-spin" />
          : s.hasAnyLocked ? <Lock size={14} /> : <Unlock size={14} />}
      </button>
    </div>
  </div>
))}
  ...
  <button
    ...
    disabled={isBulkApproving || activeTerm?.isLocked || fullyApproved === totalStudents}
  ...
  <div className="grid grid-cols-2 gap-3">
    ...
    >
      {isBulkLocking ? <Loader2 size={14} className="animate-spin" /> : <Lock size={14} />}
      Lock All Grades
    </button>
    <button
    ...
    >
      {isBulkUnlocking ? <Loader2 size={14} className="animate-spin" /> : <Unlock size={14} />}
      Unlock All Grades
    </button>
  </div>
</div>
            ...
            <section className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed mb-10">
                  Freeze the database for report generation. Once locked, no teacher can modify marks.
                </p>
                  ...
                  <>
                    <div className="space-y-4 mb-10">
                      <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Current Term</p>
                      ...
                      <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                        ...
                        <span className={cn(
                          ...
                          activeTerm.isLocked ? 'bg-rose-500 text-white' : 'bg-emerald-500 text-white'
                        )}>
                    ...
                    <button
                      onClick={() => activeTerm.isLocked ? null : setShowSealConfirm(true)}
                    ...
                    >
                      <Lock size={16} />
                      {activeTerm.isLocked ? 'Term Already Locked' : 'Apply Final Seal'}
                    </button>
