...
const PulseTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="bg-slate-900 p-4 rounded-2xl shadow-2xl border border-white/10 text-white min-w-[180px]">
      ...
      {d.grade && (
        <p className="text-[11px] font-bold text-slate-300">Grade: {d.grade}</p>
      )}
      {d.position && (
...
export function StudentDashboard() {
  ...
  const [selectedSubjectId, setSelectedSubjectId] = React.useState<
    string | null
  >(null);
  const [downloadingTermId, setDownloadingTermId] = React.useState<
    string | null
  >(null);
  const [isExporting, setIsExporting] = React.useState(false);
  const [exportStep, setExportStep] = React.useState(0);
  const [activeTab, setActiveTab] = React.useState<
    "overview" | "reports" | "subjects"
  >("overview");
  ...
  React.useEffect(() => {
    async function loadProfile() {
      ...
      try {
        ...
        try {
          const portalRes = await api.get(
            `/portal/students/${user.id}/portal-data`,
          ); // ← backtick fixed
  ...
  const topSubject = subjectMap.reduce<(typeof subjectMap)[0] | null>(
    (best, s) => {
      const avg =
  ...
  const weakestSubject = subjectMap.reduce<(typeof subjectMap)[0] | null>(
    (worst, s) => {
      const avg =
        s.grades.reduce((sum, g) => sum + (g.totalScore ?? 0), 0) /
        (s.grades.length || 1);
      const worstAvg = worst
        ? worst.grades.reduce((sum, g) => sum + (g.totalScore ?? 0), 0) /
          (worst.grades.length || 1)
        : 999;
  ...
  const renderPulseChart = () => {
    const avg = pulseData.length
      ? pulseData.reduce((sum, d) => sum + d.score, 0) / pulseData.length
      : null;
    ...
    return (
      <div className="space-y-6">
          ...
          <div className="flex items-center justify-between mb-2">
            <div>
              ...
              <p className="text-xs font-bold text-gray-400 mt-1">
                Average score across all subjects per term
              </p>
            </div>
            {latestRC && prevRC && (
              <div className="text-right">
                <DeltaBadge
                  current={latestRC.averageScore}
                  previous={prevRC.averageScore}
                />
        ...
        {/* Subject comparison — ALL subjects, no switching needed */}
        <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs font-bold text-gray-400 mt-1">
                Latest term score for each subject
              </p>
            </div>
          ...
          {subjectMap.length === 0 ? (
            ...
            <div className="space-y-4">
              {/* Sort by latest score descending so best subjects are at top */}
              {[...subjectMap]
                .sort((a, b) => {
                  const aLatest =
                    a.grades[a.grades.length - 1]?.totalScore ?? 0;
                  const bLatest =
                    b.grades[b.grades.length - 1]?.totalScore ?? 0;
                  return bLatest - aLatest;
                })
                .map((sub) => {
                  const latestGrade = sub.grades[sub.grades.length - 1];
                  const prevGrade = sub.grades[sub.grades.length - 2];
                  const score = latestGrade?.totalScore ?? 0;
                    ...
                    <div key={sub.id} className="flex items-center gap-4">
                      ...
                      <div className="w-36 shrink-0">
                        ...
                        <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">
                          {sub.grades.length} term
                          {sub.grades.length !== 1 ? "s" : ""}
                        </p>
                      </div>

                      {/* Bar */}
                      <div className="flex-1 relative h-7 bg-gray-50 rounded-xl overflow-hidden border border-gray-100">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${score}%` }}
                          transition={{ duration: 0.8 }}
                          className={cn(
                            "h-full rounded-xl flex items-center justify-end pr-2",
                            score >= 75
                              ? "bg-emerald-100"
                              : score >= 60
                                ? "bg-blue-100"
                        ...
                        {score < 20 && (
                          <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[10px] font-black text-gray-500">
                            {score.toFixed(1)}%
                          </span>
                        )}
                      </div>
                      ...
                      <span
                        className={cn(
                          "text-[11px] font-black px-2 py-1 rounded-lg w-10 text-center shrink-0",
                          gradeColor(latestGrade?.grade ?? ""),
                        )}
                      >
                        {latestGrade?.grade ?? "—"}
                      </span>
                      ...
                      <div className="w-14 shrink-0 flex justify-end">
                        <DeltaBadge
                          current={latestGrade?.totalScore}
                          previous={prevGrade?.totalScore}
                        />
                      </div>
                    </div>
                  );
            ...
            {activeTab === "overview" && (
              <>
                  ...
                  {[
                    {
                      ...
                      sub:
                        latestRC && prevRC
                          ...
                          : "Latest term",
                      color:
                        latestAvg >= 50 ? "text-emerald-600" : "text-rose-600",
                ...
                <div className="grid grid-cols-3 gap-8">
                  <div className="w-full">{renderPulseChart()}</div>
                  <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
                          ...
                          return (
                            <div key={sub.id}>
                              <div className="flex items-center justify-between mb-1.5">
                                <span className="text-[11px] font-black text-gray-700 truncate flex-1 mr-2">
                                  {sub.name}
                                </span>
                        ...
                        >
                          <div className="flex items-start justify-between mb-6">
                            ...
                            <div className="text-right">
                              ...
                              <DeltaBadge
                                current={rc.averageScore}
                                previous={prev?.averageScore}
                              />
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-3 gap-2 mb-6">
                            {[
                              {
                                label: "Average",
                                value: `${rc.averageScore?.toFixed(1)}%`,
                              },
                              {
                                label: "Position",
                                value: rc.classPosition ?? "—",
                              },
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                                  {sub.grades.length} term
                                  {sub.grades.length !== 1 ? "s" : ""} recorded
                                </p>
                              </div>
                            </div>
                            <div className="text-right">
                              <p className="text-xl font-black text-gray-900 italic font-display">
                                {avg.toFixed(1)}%
                              </p>
                              {latestGrade && (
                                <span
                              ...
                              return (
                                ...
                                >
                                  <span className="text-[9px] font-black text-gray-400 w-20 shrink-0 uppercase tracking-widest">
                                    T{g.term.termNumber.replace("TERM_", "")}{" "}
                                    {g.term.academicYear.label.split("/")[0]}
                                  </span>
                                  <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                    <motion.div
                                      initial={{ width: 0 }}
                                  ...
                                  <span
                                    className={cn(
                                      "text-[10px] font-black px-1.5 py-0.5 rounded-md w-7 text-center shrink-0",
                                      gradeColor(g.grade),
                                    )}
                                  >
                                    {g.grade}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8 pb-10">
          ...
          <section className="flex gap-4 -mx-6 px-6 overflow-x-auto pb-2 snap-x scrollbar-hide">
            ...
            <div className="min-w-[240px] bg-emerald-700 p-6 rounded-[2rem] snap-center text-white shadow-xl shrink-0">
              <p className="text-[9px] font-black text-emerald-200/60 uppercase tracking-widest mb-3">
                Best Subject
              </p>
            ...
            {bestTerm && (
              <div className="min-w-[240px] bg-amber-50 border border-amber-100 p-6 rounded-[2rem] snap-center shrink-0">
                ...
                <p className="text-[12px] font-black text-amber-600">
                  {bestTerm.fullTerm ?? bestTerm.term}
                </p>
              </div>
            )}
          </section>
          ...
          {sortedReportCards.length > 0 && (
            <div className="space-y-3">
              <h3 className="text-[17px] font-black text-gray-900 italic font-display">
                Report Cards
              </h3>
              {sortedReportCards.map((rc, i) => {
                const prev = sortedReportCards[i - 1];
          ...
          <div className="space-y-4">
            <h3 className="text-[19px] font-black text-gray-900 italic font-display">
              Subjects
            </h3>
            <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 no-scrollbar">
              <button
                ...
                className={cn(
                  "px-4 py-2 rounded-2xl text-[11px] font-black whitespace-nowrap border shadow-sm transition-all",
            ...
            {selectedSubject ? (
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm font-bold text-gray-600 mt-0.5 italic">
                          {g.remark || "—"}
                        </p>
                      </div>
                      <div className="text-right">
              ...
              <div className="py-12 text-center bg-white rounded-3xl border border-gray-100">
                ...
                <p className="text-[15px] font-black text-gray-900 italic font-display">
                  No grades yet
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {subjectMap.map((sub) => {
                  const avg =
                    sub.grades.reduce((sum, g) => sum + g.totalScore, 0) /
                    (sub.grades.length || 1);
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[13px] font-black text-gray-900 truncate flex-1 mr-2">
                          {sub.name}
                        </span>
                        <div className="flex items-center gap-2 shrink-0">
                          <DeltaBadge
                            current={latest?.totalScore}
                            previous={prev?.totalScore}
                          />
                          <span
                            className={cn(
                              "text-[10px] font-black px-1.5 py-0.5 rounded-md",
                              gradeColor(latest?.grade),
                            )}
                          >
                            {latest?.grade ?? "—"}
                          </span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-black text-gray-500 w-10 text-right">
                          {avg.toFixed(1)}%
                        </span>
                      </div>
                    </div>
          ...
          <div className="bg-emerald-800 p-8 rounded-[2.5rem] text-white space-y-4 relative overflow-hidden shadow-2xl">
            <div className="space-y-3">
              ...
              <p className="text-[10px] text-emerald-300/50">
                {9 - (profile?.reportCards?.length ?? 0)} term
                {9 - (profile?.reportCards?.length ?? 0) !== 1 ? "s" : ""}{" "}
                remaining in your SHS journey
              </p>
            </div>
          </div>
        </div>
